...
class Version:
    def __init__(self, len: int = 3, fill_func: Callable = default_zero, split_alpha: str = '.') -> None:
        self._split_num = len
    ...
    def version(self, version: str = "") -> None:
        ...
        if len(tmp_version) <= self._split_num:
            for index in range(self._split_num):
                self._ver[index] = self._fill_func(index) if index >= len(tmp_version) else tmp_version[index]
        
        else:
            raise ValueError("给定字符串分割后的长度 {} 大于设定的长度 len：{}".format(len(tmp_version), self._split_num))
        
    def is_valid(self) -> bool:
        if (
            len(self._ver) != self._split_num or
            self.version_str == ""
        ):
            return False
        return True
